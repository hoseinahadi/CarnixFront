...
const CartStep3: React.FC<CartStep3Props> = ({
    ...
    const fetchPaymentMethods = async () => {
        ...
        try {
          ...
          if (Array.isArray(data)) {
            ...
            methods = data.data;
          } else if (Array.isArray(data?.mainResults)) {
            methods = data.mainResults;
          }

          methods = methods.filter((method) => {
              const mId = method.paymentMethodId || method.id;
              return Number.isFinite(Number(mId)) && Number(mId) > 0;
          });
          ...
          setPaymentMethodsError('در حال حاضر روش پرداخت فعالی در دسترس نیست.');
        } catch (error) {
          if (cancelled) return;
          ...
          setSelectedPaymentMethodId(null);
          setSelectedPaymentType('');
          setPaymentMethodsError('دریافت روش‌های پرداخت با خطا مواجه شد. لطفاً دوباره تلاش کنید.');
        } finally {
          if (!cancelled) {
  ...
  const handleApplyCoupon = async () => {
      ...
      if (!normalizedCode || couponLoading || couponApplied) {
        return;
      }
      ...
      try {
        ...
        setCouponMessage(result?.message || 'کد تخفیف با موفقیت روی سبد خرید اعمال شد.');
      } catch (error: unknown) {
  ...
  const validateCartBeforeOrder = async (cartId: number): Promise<boolean> => {
      try {
        const response = await CartApi.validateCart(cartId);
        const result = response.data;

        if (!result || result.isSuccess !== true) {
          setErrorMessage(
            result?.message || 'سبد خرید معتبر نیست. لطفاً سبد خود را بررسی کنید.',
          );
          return false;
        }
        ...
        if (result.data === false) {
          setErrorMessage(
            result.message || 'سبد خرید معتبر نیست. قیمت یا موجودی یکی از کالاها تغییر کرده است.',
          );
          return false;
        }
        ...
        if (
          ...
          result.data !== null &&
          'isValid' in result.data &&
          (result.data as { isValid?: boolean }).isValid === false
        ) {
          setErrorMessage(result.message || 'سبد خرید نیاز به بررسی مجدد دارد.');
          return false;
        }
  ...
  const handlePayment = async () => {
      if (actionLoading || validatingCart) return;
      ...
      try {
        const cartIsValid = await validateCartBeforeOrder(cartId);
        ...
        setErrorMessage('پاسخ ثبت سفارش معتبر نیست.');
      } catch (error: unknown) {
        setErrorMessage(
          typeof error === 'string'
            ? error
  ...
  const paymentUnavailable =
    ...
    (paymentMethods.length === 0 || selectedPaymentMethodId === null);

  const checkoutBusy = actionLoading || validatingCart;
        ...
        <div className={styles.mainSection}>
                  ...
                  return (
                    <div
                      ...
                      className={`${styles.paymentMethod} ${
                        selectedPaymentMethodId === mId ? styles.selected : ''
                      }`}
                      onClick={() => {
                        setSelectedPaymentMethodId(mId || null);
          ...
          <div className={styles.couponSection}>
            ...
            <div className={styles.couponInputGroup}>
              ...
              >
                {couponLoading ? 'در حال اعمال...' : couponApplied ? '✓ اعمال شد' : 'اعمال'}
              </button>
            </div>
            {couponMessage && (
              <p className={styles.couponSuccess}>{couponMessage}</p>
            )}
          </div>
        ...
        <div className={styles.summary}>
            ...
            {cartDiscountTotal > 0 && (
              <div className={styles.summaryRow}>
                ...
                <span className={styles.discount}>
                  -{formatCurrency(cartDiscountTotal)} تومان
                </span>
              </div>
            )}
          ...
          <div className={styles.actions}>
            ...
            <button
              ...
              disabled={checkoutBusy || loadingPaymentMethods || paymentUnavailable}
            >
              {validatingCart
